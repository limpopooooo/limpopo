...
class Question:
    def __init__(
        ...
        if not isinstance(topic, str):
            raise QuestionParameterWrongType(
                "Field `topic`: is expected type `str`, received: {}".format(type(topic))
            )

        if not isinstance(strict_choose, bool):
            raise QuestionParameterWrongType(
                "Field `strict_choose` is expected type `bool`, received: {}".format(type(strict_choose))
            )

        if not isinstance(column_count, int):
            raise QuestionParameterWrongType(
                "Field `column_count`: is expected type `int`, received: {}".format(type(column_count))
            )
        ...
        if not self._validate_choices(choices):
            raise QuestionChoicesWrongType(
                "Non empty dict or list of string or ANY is expected, received: {}".format(choices)
            )
    ...
    def validate_answer(self, answer: Answer):
        if (self.strict_choose
                and self.choices != ANY
                and answer.text not in self.options):
            raise QuestionWrongAnswer("Question: {}\nchoose invalid answer: {}".format(self.topic, answer.text))
